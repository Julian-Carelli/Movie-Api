...
const foundGenders = (movie, genderList, foundedGenders) => {
    try {
        const genresIds = movie & movie?.genres || movie?.genre_ids ? movie?.genres || movie?.genre_ids : []
        genresIds.map((id) => {
...
const Card = (props:Iprops) => {
    ...
    //onclick con una accion para meterlo adentro del objeto y despues de eso un redirect hacia la pagina
    //tener en cuenta que se tienen que crear dos paginas mas y borrar la que esta
    // deberia ser /movie/{nombre} y /serie/{nombre}
    
    return (
        <div className='Card' key={movie?.id} style={{padding: "0px 40px"}}>
            <div className="Card__image">
                <img onClick={() => goToPageDetail(movie, contentTypeCustom)} src={existsImage(movie)} alt={movie?.name} style={{width:"300px", cursor: "pointer", height: "400px", borderRadius: "10px"}}/>
            </div>
            ...
            <div className="Card__action" style={{display: "flex"}}>
                <div>
                    <FontAwesomeIcon icon={faTrash} onClick={() => value.actions.deleteToFavorites(movie)}/>
                </div>
                <div>
                    <FontAwesomeIcon icon={faHeart} style={{ marginLeft: "20px" }} onClick={() => value.actions.addToFavorites(movie)}/>
                </div>
                <div>
